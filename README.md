...
### Событийная модель (по требованию)
...
- **Назначение**: Thread-safe сервис для вероятностных вычислений
...
### CsvInitialAgeReader
- **Назначение**: Adapter для преобразования CSV данных в доменные модели
...
### CsvDeathRulesReader  
- **Назначение**: Data Mapper для трансформации CSV строк в объекты DeathRule
...
### CsvResultWriter
- **Назначение**: Strategy pattern для различных форматов вывода результатов
...
### DeathRule
- **Назначение**: Value Object с правилами смертности
- **Свойства**:
...
### DemographicStats
- **Назначение**: DTO (Data Transfer Object) для статистики года
- **Свойства**:
  - `Year`, `TotalPopulation`, `MalePopulation`, `FemalePopulation`
- **Использование**: Сериализуемый объект для передачи между слоями

### ChildBirthEventArgs
- **Назначение**: Event Args для передачи контекста рождения
- **Свойства**: `ChildGender`, `BirthYear` - вся необходимая информация для создания нового Person

### SimulationResult
- **Назначение**: Aggregate Root для результатов симуляции
- **Свойства**: `YearlyStatistics` - полная история демографических изменений
